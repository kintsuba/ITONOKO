    ...
    >
      ...
      >
        <div class="grid md:grid-cols-3 gap-4 items-center">
          ...
          <ItonokoInput placeholder="切断" v-model="inputName" />
        </div>
      </div>
    </div>
              ...
              >
                ...
                <td class="pr-2 text-sm text-center text-gray-800 w-1">
                  <OutlinedButton
                    ...
                    @click="$router.push('/attribute/edit/' + attribute.id)"
                  >
...
const { data: attributes } = await useFetch("/api/attributes/get", {
  initialCache: false,
});
...
const filteredAttributes = computed(() => {
  return attributes.value.filter((attribute) =>
    attribute.name.includes(inputName.value)
  );
});
</script>
